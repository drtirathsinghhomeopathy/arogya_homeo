...
export default function PatientEdit({ user }) {
  ...
  const followupTotalPages = Math.max(1, Math.ceil(followups.length / rowsPerPage));
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 md:px-8 py-4 sm:py-6 md:py-8 space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Patient Details</h1>
            <p className="text-sm text-gray-500 mt-0.5">View and edit patient information</p>
          </div>
        </div>
    ...
    name="Address"
      ...
      {followups.length > 0 && (
        <div className="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden">
          <div className="px-4 sm:px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-primary/5 to-primary/10 flex flex-col sm:flex-row sm:flex-wrap gap-3 sm:justify-between sm:items-center">
            <div>
              <h2 className="font-semibold text-gray-800">Patient Follow-Ups</h2>
              <p className="text-sm text-gray-500">{followups.length} visit(s) recorded</p>
            </div>
            <div
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50/80">
                  ...
                  <th className="text-left py-3 px-6 font-semibold text-gray-600">Complains</th>
                  <th className="text-left py-3 px-6 font-semibold text-gray-600">Investigation</th>
                  <th className="text-left py-3 px-6 font-semibold text-gray-600">Medicine</th>
                  <th className="text-left py-3 px-6 font-semibold text-gray-600">Bill</th>
                  <th className="text-left py-3 px-6 font-semibold text-gray-600">Paid</th>
                </tr>
              </thead>
              <tbody>
                {paginatedFollowups.map((f, i) => (
                  ...
                  >
                    <td className="py-3 px-6 text-gray-600">{f.createdAt?.toDate().toLocaleDateString()}</td>
                    <td className="py-3 px-6">{f.presentingComplains}</td>
                    <td className="py-3 px-6">{f.investigation}</td>
                    <td className="py-3 px-6 max-w-[200px] truncate" title={f.medicine}>{f.medicine}</td>
                    <td className="py-3 px-6 font-medium text-gray-700">₹{f.bill}</td>
                    <td className="py-3 px-6 font-medium text-gray-700">₹{f.paid}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              >
                ...
                <div className="text-sm text-gray-600 space-y-1.5">
                  {f.presentingComplains && (
                    <p><span className="font-medium text-gray-700">Complains:</span> {f.presentingComplains}</p>
                  )}
                  {f.investigation && (
                    <p><span className="font-medium text-gray-700">Investigation:</span> {f.investigation}</p>
                  )}
                  {f.medicalHistory && (
                    <p><span className="font-medium text-gray-700">Medical History:</span> {f.medicalHistory}</p>
                  )}
                  {f.medicine && (
                    <p><span className="font-medium text-gray-700">Medicine:</span> {f.medicine}</p>
                  )}
                  <div className="flex gap-4 pt-2">
                    <p><span className="font-medium text-gray-700">Bill:</span> ₹{f.bill}</p>
                    <p><span className="font-medium text-gray-700">Paid:</span> ₹{f.paid}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="px-4 sm:px-6 py-4 border-t border-gray-100 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between text-sm text-gray-600">
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <p>
                ...
                {Math.min(followupStartIndex + rowsPerPage, followups.length)} of {followups.length} follow-ups
              </p>
              <div className="flex items-center gap-2">
                <label htmlFor="followup-rows-per-page" className="text-gray-600 whitespace-nowrap">
                  Rows:
                </label>
                <input
            ...
            {followups.length > rowsPerPage && (
              <div className="flex items-center justify-center gap-2">
                <button
                  type="button"
                  onClick={() => setFollowupCurrentPage((p) => Math.max(1, p - 1))}
                ...
                <button
                  type="button"
                  onClick={() => setFollowupCurrentPage((p) => Math.min(followupTotalPages, p + 1))}
      ...
      {followups.length === 0 && (
        <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-12 text-center">
          ...
          <p className="text-sm text-gray-400 mb-4">Add the first follow-up visit for this patient</p>
          <button
            onClick={() => setShowFollowupModal(true)}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
          <div className="bg-white w-full max-h-[95vh] sm:max-h-[85vh] sm:max-w-3xl sm:rounded-2xl rounded-t-2xl shadow-xl border border-gray-100 overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-primary/5 to-primary/10">
              ...
              <p className="text-sm text-gray-500">Record a new visit for {form.Name || "this patient"}</p>
            </div>
